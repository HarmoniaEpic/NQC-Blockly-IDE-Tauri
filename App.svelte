<script>
  ...
  async function showMessage(text, title = 'NQC Blockly IDE', type = 'info') {
    // Tauri v2ではメッセージダイアログが組み込まれていないため、
    // カスタムダイアログを使用するか、console.logを使用
    if (type === 'error') {
  ...
  async function loadSerialPorts() {
    try {
      ...
      await showMessage(error.toString(), 'エラー', 'error');
    }
  }
  ...
  async function compileCode() {
    try {
      ...
      if (result.success) {
        await showMessage('コンパイル成功', '成功');
      } else {
        await showMessage(`コンパイルエラー:\n${result.stderr}`, 'エラー', 'error');
      }
    } catch (error) {
      await showMessage(error.toString(), 'エラー', 'error');
    }
  ...
  async function downloadToRCX() {
    try {
      ...
      if (result.success) {
        ...
        await showMessage('RCXへの転送成功', '成功');
      } else {
        await showMessage(`転送エラー:\n${result.stderr}`, 'エラー', 'error');
      }
    } catch (error) {
      await showMessage(error.toString(), 'エラー', 'error');
    }
  ...
  async function controlRCX(action) {
    try {
      ...
      if (result.success) {
        await showMessage(`${action}コマンド実行成功`, '成功');
      } else {
        await showMessage(`実行エラー:\n${result.stderr}`, 'エラー', 'error');
      }
    } catch (error) {
      await showMessage(error.toString(), 'エラー', 'error');
    }
  ...
  async function saveProject() {
    try {
      ...
      if (filePath) {
        ...
        await showMessage('プロジェクトを保存しました', '成功');
      }
    } catch (error) {
      await showMessage(error.toString(), 'エラー', 'error');
    }
  ...
  async function loadProject() {
    try {
      ...
      if (filePath) {
        ...
        await showMessage('プロジェクトを読み込みました', '成功');
      }
    } catch (error) {
      await showMessage(error.toString(), 'エラー', 'error');
    }
  ...
  async function exportNqcCode() {
    try {
      ...
      if (filePath) {
        ...
        await showMessage('NQCコードをエクスポートしました', '成功');
      }
    } catch (error) {
      await showMessage(error.toString(), 'エラー', 'error');
    }
  ...
  async function clearWorkspace() {
    const confirmed = confirm('ワークスペースをクリアしますか？');
